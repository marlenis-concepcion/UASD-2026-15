...
OUT = ROOT / "Testing4All_Transformacion_QA_IA_Empleabilidad_2026_LETRA_GRANDE_V5.pptx"
...
def txt(slide, value, x, y, w, h, size=22, color=INK, bold=False,
    ...
    # Mantiene el texto dentro de su marco. PowerPoint conserva el tamaño
    # solicitado y solo lo reduce cuando realmente no cabe en width/height.
...
def cards(slide, data, top=1.55, cols=3, title_size=None, body_size=None,
          body_top=.82, body_bottom=.18, body_inset=.28):
    ...
    for i, (title, body, color) in enumerate(data):
        ...
        resolved_title_size = title_size or (20 if rows == 1 else 18)
        resolved_body_size = body_size or (18 if rows == 1 else 16)
        txt(slide, title, x+.28, y+.24, w-.52, .52, resolved_title_size, INK, True)
        txt(slide, body, x+body_inset, y+body_top,
            w-(body_inset+.20), h-(body_top+body_bottom),
            resolved_body_size, MUTED)
...
def book_idea(slide, value, y=6.40):
    ...
    txt(slide, value, 1.02, y+.08, 11.30, .42, 17, PURPLE, True,
...
def resource(slide, y, label, description, url, color=PURPLE):
    ...
    txt(slide, label, .98, y+.10, 3.0, .34, 15, color, True, url=url)
    txt(slide, description, 4.05, y+.10, 7.95, .40, 14, MUTED, url=url)
...
def pill(slide, value, x, y, w, color):
    ...
    txt(slide, value, x+.06, y+.06, w-.12, .22, 11, WHITE, True, PP_ALIGN.CENTER)
...
], cols=3, title_size=20, body_size=18)
...
txt(s, "Producto digital · Equipo ágil · Modalidad híbrida", 1.00, 2.18, 6.60, .34, 14, MUTED)
...
], 1.10, 3.35, 6.72, 2.75, 18.5, INK, CYAN)
...
txt(s, "La técnica importaba, pero también la comunicación y el conocimiento del producto.", 9.18, 5.28, 2.92, .92, 14, RGBColor(220,225,245))
...
txt(s, "Producto con IA · Automatización moderna · Equipos multidisciplinarios", 1.00, 2.18, 6.90, .34, 14, MUTED)
...
txt(s, "No busques solamente el título del puesto: lee responsabilidades, riesgos y evidencia esperada.", 9.18, 5.20, 2.92, 1.02, 14, RGBColor(235,225,245))
...
], cols=3, title_size=18, body_size=15.5)
...
], cols=2, title_size=20, body_size=18)
...
], 1.42, 2.75, 7.25, 3.1, 20, WHITE, CYAN)
...
], cols=2, title_size=20, body_size=16,
   body_top=.78, body_bottom=.08, body_inset=.22)
...
], cols=2, title_size=21, body_size=17,
   body_top=.78, body_bottom=.08, body_inset=.22)
